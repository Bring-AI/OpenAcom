...
const { validateInput, failure, inspectTerminalEndpoint, NAME, MAX_REQUEST_BYTES } = require('./delivery');
...
async function runTerminal({ name, dataDir = path.join(os.homedir(), '.openacom'), command, args = [] } = {}) {
  ...
  // Every path this command hands out gets protected, and a protection that
  // could not be applied is recorded in the descriptor instead of being dropped.
  const protections = [protectPath(directory, { directory: true })];
  ...
  if (process.platform === 'win32') {
    endpoint = `\\\\.\\pipe\\openacom-${randomBytes(16).toString('hex')}`;
  } else {
    socketDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'openacom-'));
    protections.push(protectPath(socketDirectory, { directory: true }));
  ...
  try {
    ...
    // A named pipe has no path-based POSIX mode; icacls cannot take it either,
    // so Windows pipe access is gated by the per-terminal secret below.
    if (process.platform !== 'win32') protections.push(protectPath(endpoint, { directory: false }));
    // Stale descriptors are reported, never reused and never deleted here.
    // Create the file first, then protect it, so the acl marker also covers the
    // descriptor itself and not only its directory.
    fs.writeFileSync(descriptorPath, '', { mode: 0o600, flag: 'wx' });
    descriptorCreated = true;
    protections.push(protectPath(descriptorPath, { directory: false }));
    const acl = aggregateAcl(protections);
    fs.writeFileSync(descriptorPath, JSON.stringify({ type: 'terminal', socket: endpoint, secret, acl }, null, 2) + '\n');
    for (const protection of protections) {
      if (protection.acl !== PRIVATE) note(`Could not tighten ${protection.path}: ${protection.error} The descriptor records acl: "inherited".`);
    }
    ...
    if (error.code === 'EEXIST') {
      throw failure('DESCRIPTOR_EXISTS', `Descriptor already exists: ${descriptorPath}. ${describeDescriptorConflict(descriptorPath)} Remove that exact path only after confirming the old wrapper is stopped.`, false);
    }
    throw error;
  }
}

// M2: explain a leftover descriptor without reusing it, deleting it, or printing
// the secret inside it.
function describeDescriptorConflict(file) {
  let stat;
  try { stat = fs.lstatSync(file); } catch (error) { return `It cannot be read (${error.code || error.message})`; }
  if (!stat.isFile()) return 'It is not a regular file, so inspect it manually';
  if (stat.size > 8 * 1024) return 'It is unexpectedly large, so inspect it manually';
  let value;
  try { value = JSON.parse(fs.readFileSync(file, 'utf8')); } catch { return 'It does not parse as a descriptor, so inspect it manually'; }
  if (!value || typeof value !== 'object' || value.type !== 'terminal' || typeof value.socket !== 'string') {
    return 'It is not an OpenAcom terminal descriptor, so inspect it manually';
  }
  const acl = value.acl === 'inherited' ? ' Its permissions were never tightened (acl: "inherited").' : '';
  const issue = inspectTerminalEndpoint(value.socket);
  return issue
    ? `Its endpoint is no longer usable: ${issue.message}${acl} The previous wrapper is not reachable, so this descriptor is stale.`
    : `Its endpoint is still present, so another managed terminal is live and owns this name.${acl}`;
}

module.exports = { runTerminal, describeDescriptorConflict };
