...
// Message bodies and server output land here, so the directory is made private
// once per process (the grant inherits down to later files); protecting per send
// would spawn icacls on every call on Windows.
function logsDir() {
  const d = path.join(homedir(), '.openacom', 'logs');
  fs.mkdirSync(d, { recursive: true, mode: 0o700 });
  if (_logsAcl === null) _logsAcl = protectPath(d, { directory: true }).acl;
  return d;
}
...
function resolveExe() {
  ...
  for (const shim of [path.join(npmDir, 'opencode.cmd'), path.join(npmDir, 'opencode')]) {
    try {
      ...
      const m = txt.match(/"([^"]+opencode[^"]*\.exe)"/i) || txt.match(/(?:^|\s)(\S*opencode\.exe)\b/i);
      if (m) {
...
async function startServer(dir, { port, waitMs = 10000 } = {}) {
  ...
  const log = path.join(logsDir(), `oc-serve-${p}.log`);
...
function serverSendBackground(id, message, base, announce) {
  ...
  // The body has to stay verbatim: the detached helper below reads this file
  // back to build the POST, so a summary would deliver a summary.
  const msgFile = path.join(d, `opencode-msg-${stamp}.txt`);
  fs.writeFileSync(msgFile, message, { encoding: 'utf8', mode: 0o600 });
...
function sendBackground(id, message) {
  ...
  const d = logsDir();
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  // The body has to stay verbatim: this file is the child's stdin, so opencode
  // reads the turn straight out of it.
  const msgFile = path.join(d, `opencode-msg-${stamp}.txt`);
  fs.writeFileSync(msgFile, message, { encoding: 'utf8', mode: 0o600 });
