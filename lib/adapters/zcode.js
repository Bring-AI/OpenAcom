...
// desktop option: true = force UI delivery (error if impossible), false = headless
// only, undefined (default) = auto: CDP when reachable, headless otherwise.
function send(id, message, { timeoutMs = 600e3, noWait = false, desktop } = {}) {
  const sess = get(id);
  ...
  if (desktop === true || (desktop === undefined && cdpReachable())) {
    try {
      ...
      if (desktop === true) throw e; // forced: surface the error
      // auto: fall through to headless
    }
  }
