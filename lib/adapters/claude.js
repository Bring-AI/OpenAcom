...
// Message text and turn output both land here, so the directory is made private
...
function sendRemote(entry, message, { timeoutMs = 600e3 } = {}) {
  ...
  return `OK injected into live remote session (pid ${pid} @ ${host}); the reply streams to the desktop app - read with: openacom read ${entry.id} --agent claude`;
}
...
function send(id, message, { timeoutMs = 600e3, noWait = false } = {}) {
  ...
  if (noWait) {
    ...
    // The body has to stay verbatim: cmd.exe/sh feeds it to claude through this
    // file's stdin, and quoting it into argv would break multiline prompts.
    const tmp = path.join(d, `msg-${stamp}.txt`);
    fs.writeFileSync(tmp, message, { encoding: 'utf8', mode: 0o600 });
