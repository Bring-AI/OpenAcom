...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        limit: { type: 'number', description: 'Max rows; omit for ALL sessions (default)' },
      },
...
function listSessions(args) {
  ...
  const lim = args.limit && args.limit > 0 ? args.limit : 0; // 0 = no cap, default all
  const adapters = agent ? [ADAPTERS[agent]].filter(Boolean) : Object.values(ADAPTERS);
  for (const a of adapters) {
    if (!a.available()) continue;
    rows.push(...a.list(lim));
  }
  rows.sort((x, y) => y.mtime - x.mtime);
  return JSON.stringify(lim > 0 ? rows.slice(0, lim) : rows, null, 2);
}
...
function sendMessage(args) {
  ...
  if (!/^(zcode|claude|codex):\S+/.test(from)) throw new Error(`invalid "from" (${from}): expected "agent:sessionId"`);
  const to = String(args.to || '').trim();
  if (!to) throw new Error('"to" is required: "agent:sessionId" of the target, or "zcode:new" for a fresh session');
  const tm = to.match(/^(zcode|claude|codex):(new|\S+)$/);
