...
const TOOLS = [
  ...
  {
    name: 'send_message',
    description: 'Send a message to a session as a real user turn. from/to are mandatory "agent:sessionId" addresses (e.g. "zcode:sess_...", "claude:...", "codex:...", "opencode:ses_..."; to also accepts "zcode:new" for a fresh session). Fire-and-forget by default; wait:true blocks for the reply. opencode targets are steered directly in their exact session (never forked). The delivered message carries a [via AgentRelay · from <from>] footer so the target can reply back to the sender address.',
    inputSchema: {
      type: 'object',
      properties: {
