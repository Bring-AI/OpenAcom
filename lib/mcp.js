  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        desktop: { type: 'boolean', description: 'zcode only. Default (omitted): CDP delivery through the desktop UI when the app runs with --remote-debugging-port (live refresh + steer; the app switches to that conversation; no reply text), headless otherwise. false = silent headless with no view switch. true = force UI delivery.' },
...
function sendMessage(args) {
  ...
  else if (args.desktop === false) opts.desktop = false; // omitted = auto: CDP-first for zcode
  if (args.fresh) {
