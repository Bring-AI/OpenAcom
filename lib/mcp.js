...
const TOOLS = [
  {
    name: 'list_sessions',
    description: 'List recent sessions of the local Claude Code / Codex / ZCode agents, newest first. Returns JSON rows with agent, id, title, workspace (ssh: prefix marks a remote Claude workspace), and update time.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'send_message',
    description: 'Send a message to a session as a real user turn; the target agent processes it and its reply is returned. Synchronous (may take a while); spends tokens on the target agent and appends to its history. Remote (ssh:) Claude sessions are refused.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        fresh: { type: 'boolean', description: 'zcode only: run the message in a brand-new session instead of resuming sessionId. The new session appears in the desktop task list and can be opened to read the transcript. Returns { sessionId, reply }. If set, sessionId is not required.' },
      },
...
function sendMessage(args) {
  if (args.fresh) {
    const out = require('./adapters/zcode').sendFresh(args.message, args.timeoutMs ? { timeoutMs: args.timeoutMs } : {});
    return JSON.stringify({ ok: true, sessionId: out.id, reply: out.reply }, null, 2);
  }
  ...
  return hits[0].send(args.sessionId, args.message, opts) || '(empty reply)';
}
