...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        text: { type: 'string' },
        mode: { type: 'string', enum: ['submit', 'draft'], default: 'submit' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        noSignature: { type: 'boolean', description: 'Suppress the automatic [via OpenAcom · from ...] footer.' },
        requireRead: { type: 'boolean', description: 'Read-receipt mode: the message carries a receipt id; the target must ack via the ack_message tool (or reply containing ACK-<id>). Unacknowledged sends are redelivered automatically, 3 attempts total, then reported failed. Blocks until read/failed (see ackTimeoutMs).' },
  ...
  {
    name: 'ack_message',
    description: 'Mark a read-receipt-tracked message as read (the receipt id is in the delivered message footer). Call this after processing a message that asks for a read acknowledgment.',
    inputSchema: { type: 'object', properties: { id: { type: 'string', description: 'Receipt id from the message footer' } }, required: ['id'], additionalProperties: false },
  },
  {
    name: 'inbox',
    description: 'List read-receipt-tracked sent messages and their status: sent (awaiting ack), read (acknowledged), failed (no ack after 3 attempts), pending.',
    inputSchema: {
      type: 'object',
      properties: {
...
const HANDLERS = {
  ...
  relay_send: (args) => relayRequest('POST', '/messages', {
    to: args.to, target: args.target, text: args.text, mode: args.mode || 'submit',
  ...
  inbox: (args) => {
    const rows = require('./inbox').list({ status: args.status, limit: args.limit && args.limit > 0 ? args.limit : 30 });
    return JSON.stringify(rows.map((r) => ({ id: r.id, from: r.from_addr, to: r.to_addr, status: r.status, attempts: r.attempts, maxAttempts: r.max_attempts, updatedAt: r.updated_at, readAt: r.read_at || undefined, lastError: r.last_error || undefined })), null, 2);
  },
  get_paths: getPaths,
};
